...
Repo.prototype.addHook = function(url){
    var hook = {
        ...
        d = when.defer();
    request
        .post("https://api.github.com/repos/" + this.name + "/hooks?access_token=" + nconf.get('repo_tokens')[this.name])
        .send(hookString)
...
function getWebhooks(repoName){
    var d = when.defer();
    request.get("https://api.github.com/repos/" + repoName + "/hooks?access_token=" + nconf.get('repo_tokens')[repoName])
        .end(function(res){
            d.resolve(res);
        });
    return d.promise;
